...
#### ⚠️⚠️ 決定 0034 の**前提が崩れた** — 学習ストアは Zenzai の順位に届く

2つのハーネスが食い違っていた原因は、**測るタイミング**だった。学習ハーネスは確定した
直後に1回だけ変換していた。「即座」と「15秒後」の2回測るようにしたところ:
...
**学習ストアだけで、3回の確定で、1位に上がる。** 決定 0034 の冒頭の表
(「Zenzai あり → 順位は動かない」)は反映前に測っていた。

- [ ] **遅延の正体を突き止める。** 15秒は上限を測っただけで実際が何秒かも未確定。
      `commit` は既に `stopComposition()` を呼んでいるのでそれではない。
      **IME として「修正が次の打鍵に反映されない」は十分に悪い** — 決定 0034 が
      対処しようとした「効かない」より実害が近いかもしれない
- [ ] **遅延が消せるなら決定 0034 の必要性を測り直す。** 個人化を丸ごと外して同じ
      測定が通るなら、fork・base LM(42.6MB)・世代管理はいずれも落とせる
- [ ] **落とせないなら個人化の役割を書き直す。** 40回確定の「即座」列で個人化ありだけが
      1位になっているので、「届かないものを届かせる」ではなく「反映を速くする」かもしれない

**実装は現状のまま。** 動いているものを測り直す前に外すのは順序が逆。

#### `build-and-run-personalization.ps1` の対照が通らない → 上の遅延で説明がつく
