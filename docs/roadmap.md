...
**上記の修正が原因ではない** — 未変更の `main` で確認済み。潰した仮説:

- 個人化(世代が無ければ `personalizationMode` は nil)
...
- [ ] **かな素通しの候補が実変換より上に来やすい。** `おはぎー` が `Ohagey` より上、
      `きしゃのきしゃ` が全長候補の3位。Zenzai のスコアが効いていない方向を示唆するが
      確証は無い。**次に着手するならここ**
