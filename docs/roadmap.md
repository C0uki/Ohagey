...
- [x] **llama.cpp の自前ビルドをやめた。** `tools/fetch-backends.ps1` が
  ...
  - [x] **CUDA を同梱する判断。`fetch-backends.ps1` が cudart も取るようにした。**
        `-Backends cuda` は2アーカイブを取る(azooKey の 429MB + ggml-org の cudart 548MB
        = **977MB**、ダウンロード 557MB)。片方だけでは「あるのに読み込めない」状態に
        なるので必ず一緒に取る。RTX 3050 Ti で `reason=requested` を実機確認
  - [ ] ⚠️ **ただし速度差は 12ms しかなかった**(release、同一セッション、20回):
        ...
        **977MB を配って約15%。** Zenzai は 500M 級で1回の変換が数十msなので、GPU に
        載せても稼げる幅が小さい。判断は下りているが、この数字は判断の前には無かった
  - [ ] **Vulkan のレイテンシを測る。** zip 21.8MB で NVIDIA / AMD / Intel を一括で
        賄える。CUDA と同程度なら **1/45 のサイズで足りる**ことになる
