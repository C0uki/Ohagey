...
- [ ] 選択肢2: コーパスの最小サイズを条件にする → 820行で救えなかったので**効かない見込み**
- [ ] **選択肢3: base LM の較正を疑う。** 混合は `alpha * (log p_personal - log p_base)`。
      個人モデルは5-gram・数百行、base は 42.6MB の汎用モデル。桁の違うものを引き算して
      いるので、`p_personal` が疎な語で極端な値を出している可能性がある。**未調査。本筋**
