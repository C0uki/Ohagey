...
- [ ] **820行のコーパスを入れた行が取れていない**(実行が時間切れ)。豊かなコーパスで
      漏れの「数」は減っていたので、壊れる数も減る可能性はある。**次に着手するならここ**。
      alpha は 1.00 だけに絞ってよい
- [ ] **α=1.0 で目標自身が 2位→6位 に落ちた件を確認する。** 同条件の以前の測定では
      2位→1位 だった。評価セットの32回の変換が前後に挟まったことで非同期の再学習との
      重なり方が変わったのだと思われるが、確認していない。**評価セットの有無で結果が
      変わるなら、ハーネスがまだ何かを取り違えている**
- [ ] その上で**コーパスの最小サイズを条件にする**案(学習が薄いうちは個人化を
      適用しない)を検討する。「壊れる」局面をそもそも避けられる

**既定値はまだ変えていない。** 方向は見えているが、820行の行を埋める前に動かすのは
「1条件で結論を出す」という同じ間違いになる。
