...
### ⚠️ 決定 0034 の前提が揺らいでいる(要再検討)

決定 0034 は「**学習ストアは Zenzai の順位に届かない**」という観測の上に立っている。
そのために fork を2つ切り、個人化 n-gram を実装した。

**その観測が正しかったか確認できていない。**

原因は2つ重なっていた。

1. **`ggml-org/llama.cpp` ではモデルが読めない。** zenz の日本語 char pre-tokenizer は
   `azooKey/llama.cpp` の fork にしか無い(`docs/local-setup.md` 参照)。
2. **エンジンの `zenzai_used` はモデル**ファイルの存在**だけを見ていた。** llama が
   実際に読めたかは一切見ていなかったので、Zenzai が死んでいても
   `zenzai_used=true` を返していた。ハーネスの
   `[PASS] the conversion went through Zenzai` はこれを見ていた。

2 は修正済み(読み込み失敗を検出してログに出す)。1 は fork をビルドして解消した。

**その状態で測り直した結果:**

```
before: 貴社の貴社 / 貴社の記者 / 記者の記者 / 貴社の / キシャノキシャ
after:  キシャノキシャ / きしゃのきしゃ / きしゃ / 期しゃ / 記者
```

- 5位の候補を 40 回確定 → **1位に上がる**(決定 0034 の目的は達成されている)
- しかし**残りの候補も総入れ替え**になる
- **個人化を切っても結果が同じ。** つまりこの並べ替えは個人化ではなく
  **変換器の学習ストア**によるもの
- `alpha` を 15% → 5% にしても変化なし

つまり、**Zenzai が本当に動いている状態では学習ストアが順位に効いている**可能性が高い。
そうであれば決定 0034 の個人化機構は、存在しない問題を解こうとしていたことになる。

決めるべきこと:

- [ ] Zenzai を動かした状態で、**個人化なしの学習ストアだけ**で十分かを測る。
      十分なら決定 0034 を撤回し、fork も個人化も畳めるかを検討する
      (fork には SwiftyMarisa の実バグ修正2件が入っているので、そちらは別途扱う)
- [ ] 不十分なら、`alpha` を実測で較正し直す。**現在の較正は合成コーパスで行ったもので、
      実 Zenzai に対しては未検証**
