...
##### ⚠️ 判断が要る — 出荷物では個人化が動かない

`installer/ohagey.iss` は zenz の gguf しか取得せず、**base LM はどこにも入らない**。
`OHAGEY_BASE_LM_PATH` は release では読まれない。したがって**通常のインストールでは、
設定で個人化を ON にしても何も起きない。** 決定 0034 の追記11(既定オフ、選んだら効く)は、
いまの出荷物では後半が成立していない。
...
- [ ] **2. base が無いときは個人化を適用しない**(`personalizationMode` が nil を返す)。
      効かないと分かっている経路のために確定のたびにコーパスを書き学習を回すのをやめる。
      決定 0025 とも整合する

どちらも決定 0034 / 0009 に触るので実装は変えていない。
