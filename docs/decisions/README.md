...
| 0035 | [設定スキーマの確定](0035-settings-schema.md) | 決定0014の未確定部分。**HKCU `Software\Ohagey`** に確定(ファイル方式は実装済み・検証済みだったが捨てた — Windows の IME としての慣例と、将来の GPO 配備の余地)。壊れた値は**その項目だけ**倒し、**範囲外はクランプ**する — 1つの値のせいで全体を既定値に戻すと**学習をオフにしたユーザーが黙ってオンに戻る**(決定0025)。alpha は浮動小数点型が無いので**パーセントの DWORD**。Core は Windows 非依存のまま(値の意味は Core、I/O だけ Windows)。キーが無ければ**空のまま作る** — 監視できないと新規プロファイルで設定変更に永久に気付かないため |

| 0036 | [ユーザー辞書のフォーマット](0036-user-dictionary-format.md) | 決定0026の未確定部分。`%LOCALAPPDATA%\Ohagey\userdict.tsv` の**タブ区切りテキスト**。JSON は構文エラー1つで全エントリを失うが、何年分かの手入力が入りうるファイルでそれは許容できない — 行指向なら壊れた行だけで済む(決定0035と同じ規則)。品詞は Ohagey の名前で書き、変換器の数値 ID への対応付けはコード側に置く(upstream のテーブルに辞書ファイルを依存させない)。LOUDS ビルダーは upstream の CLI ターゲットにあり呼べないので `importDynamicUserDict` を使う。**実測で3つ踏んだ**: `ruby` はカタカナ必須(ひらがなだと黙って一致しない)、辞書入替後は `stopComposition()` が要る(直前のラティスが再利用され登録語が出ない)、コストは -10(組み込みの -14.5〜-19 に勝つ) |
## 実装フェーズで詰める残課題
- ~~ユーザー辞書ファイルの具体的なフォーマット(JSON等)~~ → **決定0036で確定**(タブ区切り)
- ~~設定用のレジストリスキーマの詳細(決定0014関連)~~ → **決定0035で確定**
