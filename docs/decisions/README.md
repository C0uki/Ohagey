...
| 0037 | [個人化の学習材料の取り込み](0037-imported-training-text.md) | 決定0034の残件。個人モデルの材料が確定した語句だけ(数千字)で、引き算する相手の base は約42万字。azooKey の Tuner はこの差を**アプリをまたいだテキスト収集**で埋めているが、決定0016 / 0025 と両立しない。代わりに**利用者がテキストファイルを1つ渡す**経路を作った。`personal\imported.txt`。**扱いはユーザー辞書と同じ**(明示的に渡したものであって「打鍵の記録」ではないので、「学習データを消去」では消えず、専用の削除ボタンを持つ)。上限は**行数ではなく文字数で10万字** — 学習コストは 41µs/字・780B/字 で行数とは無関係、かつ**毎回の再学習で読み直される**ため。超過は**切り捨てではなく拒否**する(前半だけ学習して全部取り込んだと言わないため)。変更検知は `stat`(ユーザー辞書と同じ規則) |
## 実装フェーズで詰める残課題
