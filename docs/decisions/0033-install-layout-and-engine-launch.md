...
> **現状 DLL は `SampleIME.dll` という名前でビルドされる。** vendoring 元の名前が
> そのまま残っているだけで、出荷名は `OhageyTSF.dll`。改名はビルド出力名を
> 変えるだけで済むが、起動処理は「自分の隣の `OhageyEngine.exe`」を見るので
> DLL の名前に依存しない。別途対応する。
