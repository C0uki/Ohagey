# Builds and runs the candidate window colour rules. Needs nothing running.
...
& cl.exe /nologo /std:c++17 /EHsc /utf-8 /W4 /WX /Zi `
    /Fo"$out\" /Fd"$out\theme.pdb" /Fe"$out\theme-selftest.exe" `
    (Join-Path $here "theme-selftest.cpp") (Join-Path $here "..\CandidateTheme.cpp") `
    /link kernel32.lib advapi32.lib
if ($LASTEXITCODE -ne 0) { throw "build failed" }

Write-Host ""
& "$out\theme-selftest.exe"
$exit = $LASTEXITCODE
