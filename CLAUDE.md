...
- ⚠️ **decision 0034 の前提が崩れている。着手前に必ず読むこと。**
  「Zenzai 有効だと学習ストアは順位に効かない」は誤りで、**効くが約15秒かかる**。
  `tsf/Ohagey/tools/build-and-run-learning.ps1` が即座と15秒後の2回測る:
  **学習ストアのみ・3回確定で 2位→1位**(即座に測ると 2位のまま)。
  決定 0034 の個人化一式(fork / base LM 42.6MB / n-gram 学習 / 世代管理)が
  必要かどうかは**測り直しが要る**。実装は現状のまま — 測り直す前に外さないこと。
  `docs/roadmap.md` と decision 0034 の追記4を参照
