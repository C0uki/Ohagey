...
- ⚠️ **個人化の A/B が、いま手元で再現しない**(decision 0034 の 2026-08-03 追記)。
  記録は「学習ストアのみは 2位のまま、個人化を足すと **2位→1位**」だが、
  同じ `build-and-run-learning.ps1` がいまは **どちらも 2位のまま**を返し、
  `build-and-run-personalization.ps1` は **FAIL** する。**巻き添えも同時に消えている**
  (30件中0件が破壊)。上げも壊しもしない状態である。
  `ConversionService` の変更を戻しても同じなので、直近の差分のせいではない。
  同じ実行で `build-and-run-stability.ps1` が **69ms**(記録は 137ms)を出しており、
  **数字が2つ同時にずれている。原因は未特定 — 個人化の議論はこれが先。**
...
- **変換は同じ入力に同じ答えを返す**(`build-and-run-stability.ps1`)。
  ...
  **代償は速度**: release/CPU で平均 137ms、最悪 191ms
  ...
  ⚠️ ただし 2026-08-03 の再測定は **69ms / 81ms**。上の再現しない件と同じ実行での値で、
  **どちらが正しいか未確定**
