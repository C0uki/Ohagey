...
- ⚠️ **ただし個人化は既定値のままでは出せない。** 正解が既知の32件の評価セット
  (`tsf/Ohagey/tools/eval-set.tsv`)で測ると、**1つの語句を40回確定しただけで
  無関係な30件中18件が正解を失う**(α=1.0)。α=0.5 なら3件だが目標も動かない。
  豊かなコーパスで緩和されるかは**未測定**。decision 0034 の追記8 と
  `docs/roadmap.md` を参照。**alpha の既定値は測り終えるまで動かさないこと**
