...
- 🔴 **個人化には base 言語モデルが要る。無いと「弱く効く」のではなく完全に無効**
  (decision 0034 の 2026-08-04 追記)。40回確定しても順位は動かず、
  30件の巻き添えも0件。**インストーラで `Miwa-Keita/base_n5_lm` の4ファイル(42.6MB)を
  `{app}\models\` に取得するようにした**(decision 0008 の追記)。
  ⚠️ このモデルは**ライセンス未記載**。立場は decision 0009 の追記(再配布しない・
  隠さない・暫定)。作者への確認は**指示待ち**。
...
- ⚠️⚠️ **個人化を有効にすると変換品質が落ちる。着手前に必ず読むこと。**
  ...
  **14.81 logits**(1語 18.05 / 82行 15.50 / 820行 18.12)。
  ...
  ハーネスは1位と先頭を共有する候補を目標に選ぶよう直してある。
  ⚠️ **昇格は未実証のまま。** 先頭を共有する目標(`機会を作る`)は上がるが、
  **個人化を切っても同じように上がる**(学習ストアが格子で昇格させている)。
  決定 0034 はこの取り違えを3回している。**昇格を見たらまず個人化を切って測り直すこと。**
  決着には「学習ストアだけでは上がらない **かつ** 1位と先頭を共有する」読みが要る。
  🔒 **base が resume 可能でなければ個人化は適用しない**ようにした。出荷した
  4ファイルの base では害のある個人モデルしか作れないため — オンにしても何もしない。
  **自前の base が要る** — 公開されている base では resume できない
  (`_c_bc` が無い)。`OhageyLMTrain` で作れる(3117行が10秒)。
