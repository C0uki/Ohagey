...
- ⚠️⚠️ **個人化を有効にすると変換品質が落ちる。着手前に必ず読むこと。**
  ...
  **14.81 logits**(1語 18.05 / 82行 15.50 / 820行 18.12)。
  ...
  ⚠️ ただし**昇格もまだ起きない**(alpha を 20 まで上げても 2位のまま・巻き添え0)。
  🔴 **upstream は候補の1文字目に個人化を掛けない**(`prefix` が空の間は
  `bulkPredict` を呼ばない)。読み全体の候補は互いに1文字目から違うので、
  **これまでの目標は原理的に動かせない位置で争っていた**。ただし1文字目を共有する
  目標(`機会をつくる`/`機会を作る`)でも動かないので、まだ説明しきれていない。
