...
- ⚠️⚠️ **個人化を有効にすると変換品質が落ちる。着手前に必ず読むこと。**
  ...
  **14.81 logits**(1語 18.05 / 82行 15.50 / 820行 18.12)。
  ...
  実装して測った: 罰を有界にすると害は減るが(18→13)、**昇格も同じ速さで消える**。
  害が出はじめる w=0.95 は昇格が起きる w=0.99 より先で、昇格が起きるのは罰の下限が
  無改造と同じ -4.6 に戻ったときちょうど。**目標は押し上げられていたのではなく、
  周りが押し下げられていた** — 較正では分離できない。fork には取り込んでいない。
  残るのは**学習材料を増やす**道だけ。
