...
- 🔴 **左文脈が実機で届いていない。** エンジンのログは毎回 `preceding 0` である。
  決定 0034 の「同じ読みが文脈で割れるのは5件中4件」は harness の値であって、
  **TSF 側は `precedingText` を常に空で送っている**。ワイヤにもエンジンにも
  実装はあるので、残っているのは TSF 側で確定済みテキストを拾って渡すことだけ
