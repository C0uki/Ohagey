...
```
src/Ohagey.Settings.Core/     UI に依存しないロジック(ビルド・テスト済み)
...
片方だけ変えると、出荷ではなくテストが落ちる。

🚧 **WinUI 本体はまだ入っていない。** .NET SDK だけではビルドできず、Visual Studio の
「Windows アプリケーション開発」ワークロードが要る(詳細と、回避策を採らない理由は
[`docs/local-setup.md`](../docs/local-setup.md) を参照)。
